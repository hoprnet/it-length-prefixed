...
describe('decode from reader', () => {
    it('should be able to decode from an it-reader', async () => {
        ...
        const stream = reader(pipe(input, lp.encode()));
        const output = await pipe(lp.decode.fromReader(stream), async (source) => await all(source));
    ...
    it('should not decode a message that is too long', async () => {
        ...
        await expect(pipe(lp.decode.fromReader(stream, { maxDataLength: 100 }), async (source) => await all(source))).to.eventually.be.rejected.with.property('code', 'ERR_MSG_DATA_TOO_LONG');
    });
});
//# sourceMappingURL=decode.from-reader.spec.js.map