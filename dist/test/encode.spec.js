...
describe('encode', () => {
    ...
    it('should re-allocate buffer pool when empty', async () => {
        ...
        const output = await pipe(input, lp.encode({ poolSize: MIN_POOL_SIZE * 1.5 }), async (source) => await all(source));
        output.forEach((o, i) => {
            const length = varint.decode(o);
            expect(length).to.equal(input[i].length);
            expect(o.slice(varint.decode.bytes)).to.deep.equal(input[i]);
    ...
    it('should encode with custom length encoder (int32BE)', async () => {
        ...
        const output = await pipe(input, lp.encode({ lengthEncoder: int32BEEncode }), async (source) => await all(source));
        output.forEach((o, i) => {
...
//# sourceMappingURL=encode.spec.js.map